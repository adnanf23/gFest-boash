        ...
        <div className="p-6 bg-gray-950 min-h-screen">
          ...
          {activeTab === 'manage' && (
              <div className="bg-gradient-to-br from-gray-800 to-gray-700 rounded-xl shadow-xl overflow-auto border border-gray-600">
                <div className="p-6 border-b border-gray-600 space-y-6 lg:flex justify-between items-center">
                  <div>
                      ...
                      {filteredPendaftaran.length === 0 ? (
                        ...
                        filteredPendaftaran.map((item) => ( 
                          <tr key={item.id} className="border-b border-gray-600/50 hover:bg-gray-700/50 transition duration-100">
                            ...
                            <td className="p-4 font-medium text-white">{item.created_at}</td>
                            {/* ⬅️ Tombol Aksi (Menggunakan Logika Conditional Rendering yang Diminta) */}
                            <td className="p-4 text-center space-x-4 flex items-centre justify-centre m-auto">
                              
                              {/* Tombol Lihat (Hanya aktif jika status DITERIMA) */}
                              <button
                                  onClick={() => handleViewDetails(item)}
                                  className={`px-3 py-1 text-sm rounded-lg transition font-medium bg-blue-600 text-white hover:bg-blue-700`}
                              >
                                  Lihat
                              </button>
...
{/* MODAL DETAIL - Improved Layout */}
{showModal && selectedPendaftar && (
    <div className="fixed inset-0 bg-[#000000d5] flex items-center justify-center p-4 z-100">
        <div className="bg-gray-800 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex justify-between items-center px-8 py-6 border-b border-gray-700">
                <h3 className="text-2xl font-bold text-white">Detail Kredensial Pendaftar</h3>
                <button
                    onClick={() => setShowModal(false)}
                    className="text-gray-400 hover:text-white transition text-3xl font-bold leading-none w-10 h-10 flex items-center justify-center hover:bg-gray-700 rounded-lg"
                >
                    ×
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-8 py-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                        <div>
                            <h4 className="text-lg font-semibold text-gray-400 mb-4 border-b border-gray-700 pb-2">
                                Informasi Personal
                            </h4>
                            <div className="space-y-3">
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">Nama Lengkap</p>
                                    <p className="text-white text-lg font-medium">{selectedPendaftar.nama_lengkap}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">No. Telepon</p>
                                    <p className="text-white text-lg">{selectedPendaftar.no_telp}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">Usia</p>
                                    <p className="text-white text-lg">{selectedPendaftar.usia} tahun</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">Tipe Pendaftar</p>
                                    <p className="text-white text-lg capitalize">{selectedPendaftar.tipe_pendaftar}</p>
                                </div>
                                {selectedPendaftar.nama_siswa && (
                                    <div>
                                        <p className="text-sm text-gray-500 mb-1">Nama Siswa</p>
                                        <p className="text-white text-lg">{selectedPendaftar.nama_siswa}</p>
                                    </div>
                                )}
                                {selectedPendaftar.kelas && (
                                    <div>
                                        <p className="text-sm text-gray-500 mb-1">Kelas</p>
                                        <p className="text-white text-lg">{selectedPendaftar.kelas}</p>
                                    </div>
                                )}
                                {selectedPendaftar.alasan && (
                                    <div>
                                        <p className="text-sm text-gray-500 mb-1">Alasan Pendaftaran</p>
                                        <p className="text-white text-lg">{selectedPendaftar.alasan}</p>
                                    </div>
                                )}
                                <div>
                                    ...
                                    <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${
                                        ...
                                        {selectedPendaftar.status.toUpperCase()}
                                    </span>
                                </div>
                                {selectedPendaftar.created_at && (
                                    <div>
                        ...
                        <div>
                            ...
                            <div className="space-y-3 mb-6">
                                <div>
                                    ...
                                    <p className="text-white text-lg font-mono bg-gray-700 px-3 py-2 rounded">
                                        {selectedPendaftar.username || 'N/A'}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">Password Default</p>
                                    <p className="text-white text-lg font-mono bg-gray-700 px-3 py-2 rounded">
                                        12345678
                                    </p>
                                </div>
                            ...
                            <div className="text-center">
                                <h4 className="text-lg font-semibold text-gray-400 mb-4">QR Code Akses</h4>
                                {selectedPendaftar.qr_code_path ? (
                                    <div className="flex flex-col items-center">
                                        <img 
                                            ...
                                            className="w-64 h-64 border-4 border-gray-600 p-2 bg-white rounded-lg shadow-xl"
                                            onError={(e) => {
                                    ...
                                    <div className="flex items-center justify-center h-64 bg-gray-700 rounded-lg">
                                        <p className="text-red-400 text-center px-4">
                                            QR Code belum digenerate<br/>atau tidak ditemukan
                                        </p>
                                    </div>
                                )}
                            </div>
